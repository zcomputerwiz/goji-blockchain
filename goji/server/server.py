...
from goji.protocols.protocol_state_machine import message_requires_reply
from goji.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
from goji.protocols.shared_protocol import protocol_version
from goji.server.introducer_peers import IntroducerPeers
from goji.server.outbound_message import Message, NodeType
from goji.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from goji.util.errors import Err, ProtocolError
from goji.util.ints import uint16
from goji.util.network import is_localhost, is_in_network
from goji.util.ssl_check import verify_ssl_certs_and_keys
...
class GojiServer:
    def __init__(
        ...
        goji_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSGojiConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSGojiConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.goji_ca_crt_path, self.goji_ca_key_path = goji_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSGojiConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.goji_ca_crt_path, self.goji_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSGojiConnection] = None
        try:
            connection = WSGojiConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSGojiConnection, on_connect=None):
    ...
    async def start_client(
        ...
        if auth:
            ...
            ssl_context = ssl_context_for_client(
                self.goji_ca_crt_path, self.goji_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
        session = None
        connection: Optional[WSGojiConnection] = None
        try:
    ...
    def connection_closed(self, connection: WSGojiConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSGojiConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSGojiConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSGojiConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSGojiConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSGojiConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use goji's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.getgoji.net/") as resp:
                    if resp.status == 200:
    ...
    def is_trusted_peer(self, peer: WSGojiConnection, trusted_peers: Dict) -> bool:
