...
from goji.daemon.server import singleton, service_launch_lock_path
from goji.server.ssl_context import goji_ssl_ca_paths, private_ssl_ca_paths
...
class Service:
    def __init__(
        ...
        proctitle_name = f"goji_{service_name}"
        ...
        goji_ca_crt, goji_ca_key = goji_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = GojiServer(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for GojiServer to be closed")
