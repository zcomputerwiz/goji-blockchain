...
from goji.protocols import full_node_protocol, introducer_protocol
...
from goji.server.address_manager_sqlite_store import create_address_manager_from_db
...
class FullNodeDiscovery:
    ...
    async def on_connect(self, peer: ws.WSGojiConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSGojiConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSGojiConnection):
    ...
    async def on_connect_callback(self, peer: ws.WSGojiConnection):
