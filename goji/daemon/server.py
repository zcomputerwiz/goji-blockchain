...
from goji.cmds.init_funcs import check_keys, goji_init
from goji.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from goji.daemon.keychain_server import KeychainServer, keychain_commands
from goji.daemon.windows_signal import kill
from goji.plotters.plotters import get_available_plotters
from goji.plotting.util import add_plot_directory
from goji.server.server import ssl_context_for_root, ssl_context_for_server
from goji.ssl.create_ssl import get_mozilla_ca_crt
from goji.util.goji_logging import initialize_logging
...
from goji.util.service_groups import validate_service
from goji.util.setproctitle import setproctitle
from goji.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Goji.")
    quit()
...
if getattr(sys, "frozen", False):
    name_map = {
        "goji": "goji",
        "goji_wallet": "start_wallet",
        "goji_full_node": "start_full_node",
        "goji_harvester": "start_harvester",
        "goji_farmer": "start_farmer",
        "goji_introducer": "start_introducer",
        "goji_timelord": "start_timelord",
        "goji_timelord_launcher": "timelord_launcher",
        "goji_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    def _build_plotting_command_args(self, request: Any, ignoreCount: bool, index: int) -> List[str]:
        ...
        command_args: List[str] = ["goji", "plotters", plotter]
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("goji daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered GOJI_ROOT
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with GOJI_ROOT: {os.environ['GOJI_ROOT']}")
    ...
    if service_command == "goji_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in goji_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    goji_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("goji_daemon")
