...
from goji.daemon.keychain_proxy import KeychainProxy, connect_to_keychain_and_validate, wrap_local_keychain
...
from goji.util.config import config_path_for_filename, load_config
from goji.util.keychain import Keychain
from goji.util.path import mkdir
from goji.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
class PlotKeysResolver:
    ...
    async def get_farmer_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'goji keys add', 'goji keys generate' or provide a public key with -f"
            )
    ...
    async def get_pool_public_key(self, keychain_proxy: Optional[KeychainProxy] = None) -> G1Element:
        ...
        if sk is None:
            raise RuntimeError(
                "No keys, please run 'goji keys add', 'goji keys generate' or provide a public key with -p"
            )
