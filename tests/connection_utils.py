...
async def disconnect_all_and_reconnect(server: GojiServer, reconnect_to: GojiServer) -> bool:
...
async def add_dummy_connection(
    server: GojiServer, dummy_port: int, type: NodeType = NodeType.FULL_NODE
    ...
    generate_ca_signed_cert(
        server.goji_ca_crt_path.read_bytes(), server.goji_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.goji_ca_crt_path, server.goji_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: GojiServer, server_2: GojiServer) -> WSGojiConnection:
    """
