...
- Thanks @jcteng for fixing a bug on the Chia DID wallet that showed 'moji:'' instead of 'moji'.
...
- Mitigate 1-3 moji dust attacks.
...
- Mainnet uses port 7444 and other constants and service names were changed for mainnet.
...
- This is a hard fork/breaking change from RC6/7. TXGJ Coins will **not** be moved forward but your plots and keys and parts of your configuration do. When you install this version before 10AM PDST on 3/16/2021 it will load up, start finding peers, and otherwise wait for the flag drop at that time to start farming. This is likely to be the last dress rehearsal for mainnet launch. Our [3/15/2021 blog post](https://www.chia.net/2021/03/15/mainnet-update.html) has more details on the current mainnet launch plan.
...
- `chia farm summary` will now display TXGJ or XGJ as appropriate.
...
- The Farm page will now no longer get stuck at 50 TXGJ farmed.
...
- We believe that the install/migrate process was bringing forward bad farming rewards receive addresses. We have attempted to stop that by only migrating RC3 and newer configurations. You can make sure you are not effected by using the Manage Farming Rewards tool mentioned above or putting a known good wallet receive address in both `xgj_target_address` sections of config.yaml.
...
- The RC5 release is a new breaking change/hard fork blockchain. Plots and keys from previous chains will work fine on RC5 but balances of TXGJ will not come forward.
...
- RC3 is a new chain to support the last major chialisp changes. TXGJ from the RC1/2 chain do not come forward to this chain but plots and keys continue to work as usual.
...
- 'chia wallet send' now takes in TXGJ or XGJ as units instead of mojis.
...
- The Beta 27 chain is a hard fork. All TXGJ from previous releases has been reset on this chain. Your keys and plots of k=32 or larger continue to work just fine on this new chain.
...
- This release has the final mainnet rewards schedule. During the first three years, each block winner will win 2 TXGJ/XGJ per block for a total of 9216 TXGJ per day from 4608 challenges per day.
...
- The cli wallet, `chia wallet`, was incorrectly displaying TXGJ balances as if they were Coloured Coins.
...
- Welcome to the new consensus. This release is an all but a full re-write of the blockchain in under 30 days. There is now only one tip of the blockchain but we went from two chains to three. Block times are now a little under a minute but there are a couple of sub blocks between each transaction block. A block is also itself a special kind of sub block and each sub block rewards the farmer who won it 1 TXGJ. Sub blocks come, on average, about every 17 to 18 seconds.
- Starting with this Beta, there are 4608 opportunities per day for a farmer to win 1 TXGJ compared to Beta 18 where there were 288 opportunities per day for a farmer to win 16 TXGJ.
...
- New consensus means this is a very hard fork. All of your TXGJ from Beta 17/18 will be gone. Your plots and keys will work just fine however. You will have to sync to the new chain.
...
- Prior to this Beta 19, we had block times that targeted 5 minutes and rewarded 16 TXGJ to one farmer. Moving forward we have epoch times that target 10 minutes and reward 32 TXGJ to 32 farmers about every 17-18 seconds over that period. This has subtle naming and UI impacts in various places.
...
- The Farm tab in the GUI is significantly enhanced. Here you have a dashboard overview of your farm and your activity in response to challenges blockchain challnegs, how long it will take you - on average - to win a block, and how much TXGJ you've won so far. Harvester and Full Node connections have moved to Advanced Options.
...
- Testnet is now running at the primary port of 57444. Update your routers appropriately. This opens 7444 for mainnet.
...
- Changed XGJ units to TXGJ units for testnet.
...
other chains.
- Keys are now derived with the EIP 2334 (m/12381/7444/a/b).
...
- The user interface now refers to chia coins with their correct currency code of XGJ.
...
- Introducer now makes sure it only sends peer addresses to peers of peers that it can reach on port 7444 or their UPnP port.
