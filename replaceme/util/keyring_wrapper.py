...
from goji.util.default_root import DEFAULT_KEYS_ROOT_PATH
from goji.util.file_keyring import FileKeyring
from goji.util.misc import prompt_yes_no
...
# the new passphrase.
DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE = "$ goji passphrase set # all the cool kids are doing it!"

MASTER_PASSPHRASE_SERVICE_NAME = "Goji Passphrase"
MASTER_PASSPHRASE_USER_NAME = "Goji Passphrase"
...
def check_legacy_keyring_keys_present(keyring: Union[MacKeyring, WinKeyring]) -> bool:
    ...
    from goji.util.keychain import default_keychain_user, default_keychain_service, get_private_key_user, MAX_KEYS
...
class KeyringWrapper:
    ...
    def _get_initial_cached_passphrase(self) -> str:
        ...
        """
        from goji.util.keychain import supports_os_passphrase_storage
    ...
    def confirm_migration(self) -> bool:
        ...
        if master_passphrase == DEFAULT_PASSPHRASE_IF_NO_MASTER_PASSPHRASE:
            ...
            print(
                "Would you like to set a master passphrase now? Use 'goji passphrase set' to change the passphrase.\n"
            )

            response = prompt_yes_no("Set keyring master passphrase? (y/n) ")
            if response:
                from goji.cmds.passphrase_funcs import prompt_for_new_passphrase
                ...
                print(
                    "Will skip setting a master passphrase. Use 'goji passphrase set' to set the master passphrase.\n"
                )
        else:
            import colorama
    ...
    def migrate_legacy_keys(self) -> MigrationResults:
        from goji.util.keychain import get_private_key_user, Keychain, MAX_KEYS
    ...
    def migrate_legacy_keyring_interactive(self):
        ...
        """
        from goji.cmds.passphrase_funcs import async_update_daemon_migration_completed_if_running
