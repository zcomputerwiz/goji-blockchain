...
def show_plots(root_path: Path):
    from goji.plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'goji plots add -d [dir]' and remove with"
        + " 'goji plots remove -d [dir]'"
        + " Scan and check plots with 'goji plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'goji init'")
...
def create_cmd(
    ...
    from goji.plotting.create_plots import create_plots, resolve_plot_keys
...
def check_cmd(
    ...
    from goji.plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from goji.plotting.util import remove_plot_directory
