...
from goji.cmds.wallet_funcs import print_balance, wallet_coin_unit
from goji.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from goji.protocols.pool_protocol import POOL_PROTOCOL_VERSION
...
from goji.util.ints import uint16, uint32, uint64
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojis = uint64(int(fee * units["goji"]))
        ...
        try:
            ...
            while time.time() - start < 10:
                ...
                if len(tx.sent_to) > 0:
                    ...
                    print(f"Do goji wallet get_transaction -f {fingerprint} -tx 0x{tx_record.name} to get status")
                    return None
        except Exception as e:
...
async def self_pool(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    func = functools.partial(wallet_client.pw_self_pool, wallet_id, fee_mojis)
    await submit_tx_with_confirmation(msg, prompt, func, wallet_client, fingerprint, wallet_id)
