...
from goji.cmds.passphrase import passphrase_cmd
...
from goji.cmds.plotnft import plotnft_cmd
from goji.cmds.plotters import plotters_cmd
from goji.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
...
from goji.util.ssl_check import check_ssl
...
@click.group(
    ...
    epilog="Try 'goji start node', 'goji netspace -d 192', or 'goji show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from goji.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
@click.option(
    ...
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by goji start <service>
)
@click.pass_context
