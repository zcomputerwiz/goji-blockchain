...
ROOT = pathlib.Path(importlib.import_module("goji").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `goji` file name (which is the executable)
    and the `goji` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `goji` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("goji-blockchain"))[0]
...
# TODO: collapse all these entry points into one `goji_exec` entrypoint that accepts the server as a parameter

entry_points = ["goji.cmds.goji"] + [f"goji.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "goji.util.bip39"])

if THIS_IS_WINDOWS:
    goji_mod = importlib.import_module("goji")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/goji/util/english.txt", "goji/util"))
datas.append((f"{ROOT}/goji/util/initial-config.yaml", "goji/util"))
datas.append((f"{ROOT}/goji/wallet/puzzles/*.hex", "goji/wallet/puzzles"))
datas.append((f"{ROOT}/goji/ssl/*", "goji/ssl"))
...
add_binary("goji", f"{ROOT}/goji/cmds/goji.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/goji/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/goji/server/start_{server}.py", COLLECT_ARGS)
