...
from goji.consensus.network_type import NetworkType
from goji.consensus.pot_iterations import calculate_iterations_quality, calculate_sp_interval_iters
from goji.farmer.farmer import Farmer
from goji.protocols import farmer_protocol, harvester_protocol
from goji.protocols.harvester_protocol import PoolDifficulty
...
class FarmerAPI:
    ...
    async def new_proof_of_space(
        self, new_proof_of_space: harvester_protocol.NewProofOfSpace, peer: ws.WSGojiConnection
    ):
        """
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            headers={"User-Agent": f"Goji Blockchain v.{__version__}"},
                        ) as resp:
                            if resp.ok:
    ...
    @api_request
    @peer_required
    async def respond_plots(self, _: harvester_protocol.RespondPlots, peer: ws.WSGojiConnection):
